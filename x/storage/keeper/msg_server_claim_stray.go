...
func (k msgServer) ClaimStray(goCtx context.Context, msg *types.MsgClaimStray) (*types.MsgClaimStrayResponse, error) {
	...
	f := false
	k.IterateActiveDeals(ctx, func(deal types.ActiveDeals) bool { // we iterate now, ideally it is not consensus breaking but should improve memory usage, this is way better in v4
		if deal.Provider == provider.Address {
			f = true
			return true
		}
		return false
