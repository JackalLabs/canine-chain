...
func SimulateMsgUpgradeStorage(
	...
	return func(r *rand.Rand, app *baseapp.BaseApp, ctx sdk.Context, accs []simtypes.Account, chainID string,
		...
		paymentInfo, found := k.GetStoragePaymentInfo(ctx, simAccount.Address.String())
		if !found {
			return simtypes.NoOpMsg(types.ModuleName, types.TypeMsgUpgradeStorage, "user have not bought any storage"), nil, nil
		}

		if ctx.BlockTime().After(paymentInfo.End) {
			return simtypes.NoOpMsg(types.ModuleName, types.TypeMsgUpgradeStorage, "old plan is expired"), nil, nil
		}
		...
		if jBalance.Amount.LTE(cost) {
			...
			if err != nil {
				return simtypes.NoOpMsg(types.ModuleName, types.TypeMsgUpgradeStorage, "unabled to fund account"), nil, err
			}
		...
		if !hasNeg {
			...
			if err != nil {
				return simtypes.NoOpMsg(types.ModuleName, types.TypeMsgUpgradeStorage, "unable to generate fees"), nil, err
			}
		}
