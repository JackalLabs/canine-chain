...
import (
	...
	minttypes "github.com/jackalLabs/canine-chain/x/ethercoat/minttypes"

	everrors "github.com/jackalLabs/canine-chain/x/ethercoat/evtypes/everrors"
...
func newAccessListTx(tx *ethtypes.Transaction) (*AccessListTx, error) {
	...
	if tx.Value() != nil {
		amountInt, err := minttypes.SafeNewIntFromBigInt(tx.Value())
		if err != nil {
			return nil, err
		}
	...
	if tx.GasPrice() != nil {
		gasPriceInt, err := minttypes.SafeNewIntFromBigInt(tx.GasPrice())
		if err != nil {
			return nil, err
		}
...
func (tx AccessListTx) Validate() error {
	gasPrice := tx.GetGasPrice()
	if gasPrice == nil {
		return sdkerrors.Wrap(everrors.ErrInvalidGasPrice, "cannot be nil")
	}
	if !minttypes.IsValidInt256(gasPrice) {
		return sdkerrors.Wrap(everrors.ErrInvalidGasPrice, "out of bound")
	}

	if gasPrice.Sign() == -1 {
		return sdkerrors.Wrapf(everrors.ErrInvalidGasPrice, "gas price cannot be negative %s", gasPrice)
	}

	amount := tx.GetValue()
	// Amount can be 0
	if amount != nil && amount.Sign() == -1 {
		return sdkerrors.Wrapf(everrors.ErrInvalidAmount, "amount cannot be negative %s", amount)
	}
	if !minttypes.IsValidInt256(amount) {
		return sdkerrors.Wrap(everrors.ErrInvalidAmount, "out of bound")
	}

	if !minttypes.IsValidInt256(tx.Fee()) {
		return sdkerrors.Wrap(everrors.ErrInvalidGasFee, "out of bound")
	}

	if tx.To != "" {
		if err := minttypes.ValidateAddress(tx.To); err != nil {
