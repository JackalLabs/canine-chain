...
func AllTxLogsFromEvents(events []abci.Event) ([][]*ethtypes.Log, error) {
	...
	for _, event := range events {
		if event.Type != evtypes.EventTypeTxLog {
			continue
		}
...
func ParseTxLogsFromEvent(event abci.Event) ([]*ethtypes.Log, error) {
	logs := make([]*maintypes.Log, 0, len(event.Attributes))
	for _, attr := range event.Attributes {
		if !bytes.Equal(attr.Key, []byte(evtypes.AttributeKeyTxLog)) {
			continue
		}

		var log maintypes.Log
	...
	return maintypes.LogsToEthereum(logs), nil
}
