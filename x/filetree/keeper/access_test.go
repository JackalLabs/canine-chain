...
func FuzzMakeEditorAddress(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, track, user string) {
		...
		eh := sha256.New()
		_, _ = fmt.Fprintf(eh, "e%s%s", track, user)
...
func FuzzMakeOwnerAddress(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, track, user string) {
		...
		eh := sha256.New()
		_, _ = fmt.Fprintf(eh, "o%s%s", track, user)
		expHash := fmt.Sprintf("%x", eh.Sum(nil))
...
func FuzzMakeViewerAddress(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, track, user string) {
		...
		eh := sha256.New()
		_, _ = fmt.Fprintf(eh, "v%s%s", track, user)
		expHash := fmt.Sprintf("%x", eh.Sum(nil))
...
func (suite *KeeperTestSuite) TestMakeViewerAddress() {
	...
	for _, tc := range cases {
		...
		suite.Run(fmt.Sprintf("trackingNum: %s, user: %s", tc.trackingNum, tc.user), func() {
			h := sha256.New()
			_, _ = fmt.Fprintf(h, "v%s%s", tc.trackingNum, tc.user)
			hash := h.Sum(nil)
	...
	for name, tc := range cases {
		suite.Run(name, func() {
			...
			if tc.owner == "" {
				...
				h1 := sha256.New()
				_, _ = fmt.Fprintf(h1, "o%s%s", tc.addr, accountHash)
				hash1 := h1.Sum(nil)
...
func FuzzHasEditAccess(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, edit, track, user string) {
		...
		if !json.Valid([]byte(edit)) {
			...
			h := sha256.New()
			_, _ = fmt.Fprintf(h, "e%s%s", track, user)
			hash := h.Sum(nil)
	...
	for name, tc := range cases {
		suite.Run(name, func() {
			if tc.editAccess == "" {
				...
				h := sha256.New()
				_, _ = fmt.Fprintf(h, "e%s%s", tc.trackingNum, tc.user)
				hash := h.Sum(nil)
