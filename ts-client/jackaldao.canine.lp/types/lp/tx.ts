...
export const MsgCreateLPool = {
  encode(message: MsgCreateLPool, writer: Writer = Writer.create()): Writer {
    ...
    if (message.ammId !== 0) {
      writer.uint32(24).uint32(message.ammId);
    }
    if (message.swapFeeMulti !== "") {
      writer.uint32(34).string(message.swapFeeMulti);
    }
    if (message.minLockDuration !== 0) {
      writer.uint32(40).int64(message.minLockDuration);
    }
    if (message.penaltyMulti !== "") {
      writer.uint32(50).string(message.penaltyMulti);
    }
    return writer;
  },
